...
if [[ $HARDWARE == AArch64* ]]; then
    ...
    python3 -m virtualenv venv --system-site-packages && source venv/bin/activate
    python3 -m pip install -q -r $script_dir/requirements_cpu_131.txt
fi
...
python3 cpu-utils/generate_yaml_for_cpu.py -j $JOB -p $HOME/.cache/torchbenchmark

#threads2cores=('16_0-15' '2_0-1' '1_0')
threads2cores=('16_0-15')
...
do
    ...
    taskset -c ${cores} python3 torchbenchmark/.github/scripts/run-config.py \
            -c CPU_${JOB}.yaml -b ./torchbenchmark/ --output-dir .
